...
認証コードやトークンは、リポジトリやteamの共有画面へ貼り付けません。

## 2. teamのリポジトリを手元に用意する

講師が共有する一覧から **自分のteam** を開きます。Organizationへの招待を承諾し、自分のteamへの参加処理が済んでいるか確認します。

GitHubの **Code → HTTPS** からURLをコピーします。作業を置きたいフォルダに移動し、次のURL部分を自分のteamのものに置き換えます。
...
上のOrganization名とteam番号は例です。フォルダ名も、自分がcloneしたリポジトリ名に合わせます。`git remote -v`に自分のteamのURLが表示されることを確認してください。
